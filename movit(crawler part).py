...
def run_the_script():
    ...
    if len(movie_date[1]) == 1:
        movie_date[1] = "0" + movie_date[1]
    if len(movie_date[2]) == 1:
        movie_date[2] = "0" + movie_date[2]
    ...
    if movie_is_on == 0:
        ...
        # 以下的far是指大遠百
        # 以下的big是指巨城
        # 當電影沒有中英兩種語言可選時，id編號是1_9；可選時是1_1(中)、1_2(英)
        far_have_available_date = movie_soup.select('article#movieTime1_9_7 div.movieDay h4')
        if far_have_available_date == []:
            far_have_available_date = movie_soup.select('article#movieTime1_1_7 div.movieDay h4')
        # print(far_have_available_date)

        big_have_available_date = movie_soup.select('article#movieTime1_9_9 div.movieDay h4')
        if big_have_available_date == []:
            big_have_available_date = movie_soup.select('article#movieTime1_1_9 div.movieDay h4')
        # print(big_have_available_date == [])
        ...
        if far_have_available_date:
            for j, date in enumerate(far_have_available_date):
                if date.text.strip().startswith(movie_date):
                    if movie_soup.select('article#movieTime1_9_7 div.movieDay') == []:
                        far_all_available_time = movie_soup.select('article#movieTime1_1_7 div.movieDay')[j].select('ul li')
                        far_all_available_time = [time.text for time in far_all_available_time]
                        far_all_available_seat_url = movie_soup.select('article#movieTime1_1_7 div.movieDay')[j].select('ul a[target="_blank"]')
        ...
        if far_the_date_have_the_movie:
            ...
            print(f'真可惜，這天新竹大遠百沒有播映《{movie_name}》。')
        ...
        if big_have_available_date:
            for k, date in enumerate(big_have_available_date):
                if date.text.strip().startswith(movie_date):
                    if movie_soup.select('article#movieTime1_9_7 div.movieDay') == []:
                        big_all_available_time = movie_soup.select('article#movieTime1_1_9 div.movieDay')[j].select('ul li')
                        big_all_available_time = [time.text for time in big_all_available_time]
                        big_all_available_seat_url = movie_soup.select('article#movieTime1_1_9 div.movieDay')[j].select('ul a[target="_blank"]')

                    else:
                        big_all_available_time = movie_soup.select('article#movieTime1_9_9 div.movieDay')[j].select('ul li')
                        big_all_available_time = [time.text for time in big_all_available_time]
                        big_all_available_seat_url = movie_soup.select('article#movieTime1_9_9 div.movieDay')[j].select('ul a[target="_blank"]')

                    big_all_available_seat_url = [seat_url.get('href') for seat_url in big_all_available_seat_url]
                    big_the_date_have_the_movie = 1
                    # print(big_all_available_time)
                    break
        ...
        if big_the_date_have_the_movie:
            ...
            print(f'真可惜，這天新竹巨城沒有播映《{movie_name}》。')
        
        print()

        while (far_the_date_have_the_movie or big_the_date_have_the_movie) != True:
            print(f'不好意思，{movie_date}當天，您所指定的電影《{movie_name}》在巨城或與大遠百威秀皆沒有播映，\n麻煩請重新輸入電影名與欲觀影時間。或是輸入"Exit"以終止程式。')
            run_the_script()
        ...
        while True:    
            ...
            if movie_city in ('f', 'far', '大遠百', '新竹大遠百', '遠百'):
                if far_the_date_have_the_movie == 0:
                    print(f"不好意思，當日新竹大遠百威秀沒有播映您所指定的電影《{movie_name}》，請選擇新竹巨城威秀影城。")
                else:
                    os.system("cls")
                    print()
                ...
                if big_the_date_have_the_movie == 0:
                    print(f"不好意思，當日新竹巨城威秀沒有播映您所指定的電影《{movie_name}》，請選擇新竹大遠百威秀影城。")
                else:
                    os.system("cls")
                    print()
                    movie_city = '新竹巨城威秀影城'
